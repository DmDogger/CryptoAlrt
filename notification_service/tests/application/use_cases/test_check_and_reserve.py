...
class TestCheckAndReserveUseCase:
    ...
    async def test_email_enabled_case(
        ...
        """
        use_case_result = await mock_check_and_reserve_use_case.execute(sample_alert_triggered_event)
        result_from_preference = await mock_fake_preference_repository.get_by_email(sample_user_preference_entity.email)

        idempotency_key_for_email = IdempotencyKeyVO.build(
            event_id=sample_alert_triggered_event.id,
            channel=ChannelEnum.EMAIL,
        )
        result_from_notification = await mock_fake_repository.get_by_idempotency_key(idempotency_key_for_email.key)
