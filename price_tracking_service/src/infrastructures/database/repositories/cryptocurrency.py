...
class SQLAlchemyCryptocurrencyRepository(CryptocurrencyRepositoryProtocol):
    ...
    async def get_by_cryptocurrency_id(
        ...
        """
        try:
            stmt = select(Cryptocurrency).where(Cryptocurrency.id == cryptocurrency_id)
            result = await self.session.scalars(stmt)
            model = result.first()
            if model is None:
                return None
    ...
    async def get_sorted_cryptocurrencies_by_created_at_time(
        ...
        """
        try:
            stmt = select(Cryptocurrency).order_by(Cryptocurrency.created_at)
            result = await self.session.scalars(stmt)
            models = result.all()
            return [self._mapper.from_database_model(model) for model in models]
        except SQLAlchemyError as e:
    ...
    async def save(self, cryptocurrency_entity: CryptocurrencyEntity) -> None:
        """
