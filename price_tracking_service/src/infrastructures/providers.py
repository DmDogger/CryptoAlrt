...
from infrastructures.database.repositories.cryptocurrency import (
...
class InfrastructureProvider(Provider):
    ...
    def get_cryptocurrency_repository(
        self, session: AsyncSession, mapper: CryptocurrencyDBMapper
    ) -> CryptocurrencyRepositoryProtocol:
        """Repository для криптовалют."""
        return SQLAlchemyCryptocurrencyRepository(session=session, mapper=mapper)

    @provide(scope=Scope.REQUEST)
    def get_alert_repository(self, session: AsyncSession) -> AlertRepositoryProtocol:
        """Repository для алертов."""
        return SQLAlchemyAlertRepository(session=session, mapper=AlertDBMapper())
    ...
    def get_coingecko_client(
        ...
        return CoinGeckoClient(client=http_client, cryptocurrency_repository=repository)
...
class UseCaseProvider(Provider):
    ...
    def get_check_threshold_use_case(
        ...
        return CheckThresholdUseCase(
            alert_repository=alert_repository, event_publisher=event_publisher
        )
    ...
    def get_publish_price_update_use_case(
        ...
        return PublishPriceUpdateToBrokerUseCase(
            broker=event_publisher, repository=cryptocurrency_repository
        )
    ...
    def get_delete_alert_use_case(
        ...
        return DeleteAlertUseCase(repository=alert_repository)
